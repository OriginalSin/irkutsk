(function () {
    ...
    var layerGroup,
		...
		arr = /dom=(\w+)/.exec(location.search),
		fromDomain = arr ? arr[1] : 'rosreestr',
		proxy = protocol + '//pkk5.' + fromDomain + '.ru/',	// http://kosmosnimki.ru/pkk5/ http://pkk5.rosreestr.ru/
		exportIcon,
		lastFeature,
		lastFeatureId,
		lastOverlays = [],
		...
		getTxt = window._gtxt || function(key) {
			var arr = key.split('.');
			return localeHash[arr[arr.length - 1]] || '';
		};
	...
	L.ImageOverlay.CrossOrigin = L.ImageOverlay.extend({
	  ...
	  exportGeometry: function () {
		...
		if (rings.length) {
			var obj = rings.length > 1 ? (L.multiPolygon || L.polygon)(rings) : L.polygon(rings[0]),
				geo = _map.gmxDrawing ? _map.gmxDrawing.add(obj) : obj;

			_map.addLayer(geo);
			if (this.options.geoLink && !notHideDrawing) { this._dObj = geo; }
			this._exObj = geo;
			this.bringToBack();
			if (_map._pathRoot) { _map._pathRoot.style.cursor = 'help'; }
			return geo;
		}
	  }
	});
	...
	*/
    ...
    L.CadUtils = {
        setOverlay: function(it, map, flagExternalGeo) {
			var attr = L.CadUtils.getFeatureExtent(it, map),
                ...
                imageUrl = proxy +  'arcgis/rest/services/Cadastre/';
				...
				.on('load', function(ev) {
					...
					if (len && target._leaflet_id === lastOverlays[len - 1]._leaflet_id) {
						...
						if (inputShowObject && inputShowObject.checked) {
							...
							if (geo && lastFeature) {
								geo.options.cadastreFeature = lastFeature;
								geo
        ...
        getFeature: function(id, cadCount, featureCont, layer, map) {
			lastFeature = null;
			L.CadUtils.requestJSONP(proxy +  'api/features/' + layer.id + '/' + id, {},
				{
				...
				if (map) { map.fire('cadastreFeature', {cadastreFeature: lastFeature}); }
			});
        },
        getFeatureExtent: function(attr, map) {
            var R = 6378137,
                ...
                latLngBounds = L.latLngBounds(
                    crs.unproject(L.point(attr.extent.xmin, attr.extent.ymin)),
                    crs.unproject(L.point(attr.extent.xmax, attr.extent.ymax))
                );
            ...
            return {
                ...
                latlng: crs.unproject(L.point(attr.center.x, attr.center.y)),
        ...
        setBoundsView: function(id, it, map, flagExternalGeo) {
        ...
        getContent: function(cadastrePkk5, popup, skipOverlay) {
			...
			if (!skipOverlay && map) { L.CadUtils.setOverlay(it, map); }
			...
			if (notHideDrawing) {
				...
				L.DomEvent.on(inputShowObject, 'click', function(ev) {
					if (!inputShowObject.checked) {
						...
						L.CadUtils.setBoundsView(id, popup._its[popup._itsCurr], map, ev.ctrlKey);
					}
				});
			} else {
			...
			exportIcon.innerHTML = L.gmx ? '<svg role="img" class="svgIcon"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#download"></use></svg>' : 'Скачать';

			exportIcon.addEventListener('click', function () {
			  var len = lastOverlays.length,
				overlay = len ? lastOverlays[len - 1] : null;
			  if (overlay && overlay._exObj && overlay._exObj._map) {
				var dObj = overlay._exObj;
        ...
        _clearLastBalloon: function(map) {
			if (this._lastOpenedPopup && this._lastOpenedPopup._map) { map.removeLayer(this._lastOpenedPopup); }
        ...
        balloon: function(ev) {
            if (ev.type === 'click' && this._map) {
                ...
                var popup = L.popup({minWidth: 350, className: 'cadasterPopup'}, cadastrePkk5)
					...
					.on('close', function() {
						lastOverlayId =
						layerGroup._cadClickLatLng = null;
					})
                    .openOn(map);

                L.CadUtils._lastOpenedPopup = popup;
				L.CadUtils.getCadastreFeatures(L.extend(ev, {url: proxy + 'api/features/', callbackParamName: 'callback'})).then(function(data) {
					...
					if (arr.length) {
						...
						for (var i = arr.length - 1; i >=0; i--) {
							...
							if (attrs.id === ev.cn || attrs.cn === ev.cn) {
								popup._itsCurr = i;
								break;
							}
						}
		...
		getCadastreFeatures: function(options) {
			if (options.latlng) {
				...
				return L.CadUtils.requestJSONP(
		...
		lastMapId: 1,
		newId: function() {
			L.CadUtils.lastMapId += 1;
			return '_cad' + L.CadUtils.lastMapId;
		},

		uniqueGlobalName: function(thing) {
			var id = L.CadUtils.newId();
			window[id] = thing;
			return id;
		},
		requestJSONP: function(url, params, options) {
			...
			return new Promise(function(resolve, reject) {
				...
				if (callbackParamName) {
					var callbackName = L.CadUtils.uniqueGlobalName(function(obj) {
    ...
    var publicInterface = {
        ...
        afterViewer: function (params, map) {
			var gmxLayers = map.gmxControlsManager ? map.gmxControlsManager.get('layers') : null;
			...
			if (!layerGroup) {
				...
				if (params.searchControl) {
					searchControl = params.searchControl;
				}
				...
				var clickOn = function(ev) {
						if (!map._skipClick && (!map.isGmxDrawing || !map.isGmxDrawing())) {
					...
					addLayer = function() {
						...
						if (!layerWMS) {
							if (L.gmx) {
								...
								layerWMS = L.tileLayer.wms(proxy +  'arcgis/rest/services/Cadastre/Cadastre/MapServer/export', {
					...
					showBalloon = function(res) {
						var prom = addLayer(),
							resolveLayer = function() {
								L.CadUtils.setBoundsView(res.attrs.cn, res, layerWMS._map);

								var featureExtent = L.CadUtils.getFeatureExtent(res, layerWMS._map);
								layerGroup._cadClickLatLng = featureExtent.latlng;
								L.CadUtils.balloon.call(layerWMS, {type: 'click', latlng: featureExtent.latlng, cn: res.attrs.cn});
							};
						if (prom) {
							prom.then(resolveLayer);
						} else {
							resolveLayer();
						}
					},
					searchHook = function(str) {
						...
						if (!it) { return false; }
						L.CadUtils.requestJSONP(proxy +  'api/features/' + it.id,
							{
								WrapStyle: 'func',
								text: str,
								limit: it.limit || 11,
							...
							if (result && result.features && result.features.length) {
								showBalloon(result.features[0]);
							} else {
								console.log(it.title + '`' + str + '`' + ' не найден!', true, 'Поиск по кадастровым номерам');
							}
						});
						return true;
					},
					toogleSearch = function (flag) {
						if (flag) {
                            ...
                            if (window.searchLogic) {
                                window.searchLogic.addSearchByStringHook(searchHook, 1001);
                                window.searchLogic.searchControl.setPlaceHolder(getTxt('$$search$$_Cadastre_1'));
                            }
                            ...
                            if (window.searchLogic) {
                                window.searchLogic.removeSearchByStringHook(searchHook, 1001);
                                window.searchLogic.searchControl.setPlaceHolder(getTxt('$$search$$_Cadastre_0'));
                            }
				...
				publicInterface.clickOn = clickOn;
				publicInterface.showBalloon = showBalloon;
                map
				...
				toogleSearch(true);
				publicInterface.searchHook = searchHook;
			} else if (gmxLayers) {
				gmxLayers.addOverlay(layerGroup, getTxt('cadastrePlugin.name'));
            }
        ...
        getPopup: function() {
            return L.CadUtils._lastOpenedPopup;
        },
    ...
    if (window.gmxCore) {
		...
		window.nsGmx.cadastre = window.publicInterface = publicInterface;
	}
})();
